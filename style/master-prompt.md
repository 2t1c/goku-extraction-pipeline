...
- **Stay close to the source. Insight comes from the story, not from author voiceover on top of it.** The Andreessen "Eternal September" post is the canonical example: every line in the body is doing one of four things — quoting Andreessen verbatim, stating a specific fact Andreessen named (Mosaic, two million users, September 1993, freshmen with college email accounts), bridging the narrative ("Then America Online bought a connection to it.", "After AOL connected, the September never ended."), or explaining the etymology of the named concept. There is almost zero generic author commentary. **Audit-trigger pattern:** sentences like *"The boring stuff compounds. The exciting stuff usually doesn't."* / *"That's a moat most companies would defend."* / *"Most platforms compete for time. Roblox competes for presence."* — these are author observations laid *on top of* the story, not insight derived *from* it. Cut them. Posts are narrative-driven AND insightful — but the insight should land through the cascade, the specific facts, and the speaker's reflective quote. Not through the writer's interpretive voiceover.
...
- **(P2) Echo the named concept across multiple scales of meaning — when the post genuinely supports it.** Optional reinforcement of move #1. When a post has a strong named concept (Eternal September, Builder's Club, tickets, holodeck), the concept can appear at multiple scales — mechanism setup → naming → live noun the reader now owns. The Andreessen post does this beautifully (mechanism: the freshman-September drift → naming: *"**Eternal September**"* → live noun: *"the September never ended"*). But many posts don't have a single dominant named concept that earns multiple echoes. **Don't force it.** Bold the concept once if it exists, let it echo naturally if the post supports it, skip the technique entirely if it's not there. A clean single-bold post lands fine without forced echoes.
